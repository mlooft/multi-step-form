(function($) {
    ...
    function renderSubmit(block) {
        return '<div class="fw-block-heading fw-block-hndle"><i class="fa fa-caret-down" aria-hidden="true"></i><h4>' + '&nbsp;' + 'Form Submit</h4></div><p>Contains a field for name and email</p>';
    }

    function renderCheckbox(block) {
        log('checkbox', block);
        var textHtml;
        textHtml = '<div class="fw-block-heading fw-block-hndle"><i class="fa fa-caret-down" aria-hidden="true"></i><h4>' + '&nbsp;' +'Checkbox</h4></div>';
    ...
    function renderTextInput(block) {
        log('textInput', block);
        var textHtml;
        textHtml = '<div class="fw-block-heading fw-block-hndle"><i class="fa fa-caret-down" aria-hidden="true"></i><h4>' + '&nbsp;' +'Text field </h4>';
        textHtml += '</div>'
    ...
    function renderTextArea(block) {
        log('textArea', block);
        var textAreaHtml;
        textAreaHtml = '<div class="fw-block-heading fw-block-hndle"><i class="fa fa-caret-down" aria-hidden="true"></i><h4>' + '&nbsp;' +'Text area</h4></div>';
    ...
    function renderStep(step) {
        var stepHtml = '<div class="postbox">' +
            ...
            '<h1 class="hndle ui-sortable-handle" title="click to collapse, drag to rearrange"><span>' +
    ...
    /**
     * insertBlock - inserts a block from param into the form.
     * the insert position is determined with the passed event
     *
     * @param  {DOM} block the block to add to the form
     * @param  {event} event  event that contains the position
     */
    function insertBlock($block, event) {
      var $part = $(event.target).parent();
      var $blocks = $part.find('.fw-step-block');
      var inserted = false;
      if ($blocks.length == 0) {
        $part.find('.fw-add-element').before($block);
      } else {
        $blocks.each(function(idx, element) {
          var $thisE = element;
          var $nextE = $blocks[idx + 1];

          if (!inserted) {
            var thisMiddleY = $($thisE).offset().top + 0.5 * $($thisE).height();
            if($nextE) {
              var nextMiddleY = $($nextE).offset().top + 0.5 * $($nextE).height();
              log ('THIS MIDDLE Y', thisMiddleY);
              // insert in between two elements
              if (thisMiddleY <= event.pageY && nextMiddleY >= event.pageY) {
                $(element).after($block);
                inserted = true;
              } else if (idx == 0 && thisMiddleY > event.pageY) {
                $(element).before($block);
                inserted = true;
              }
            } else if (idx == $blocks.size() - 1){
              $blocks.last().after($block);
              inserted = true;
            }
          }
        });
    ...
    // TODO: insert dummy on drag over
    function blockOver(event, ui) {
      // var dummy = '<div class="fw-step-block-dummy">DUMMY</div>';
      // insertBlock($(dummy), event);
    }
    ...
    function setupDragNDrop() {
        ...
        $(container + ' .fw-step-block').droppable({
          greedy: true,
          over: console.log('Hi')
        });
    ...
    function setupClickHandlers(){
      ...
      $('.fw-block-heading').click(function(e) {
