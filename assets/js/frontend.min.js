jQuery( document ).ready( function ( $ ) {
           ...
           for (part in d) {
               for (block in d[part]) {
                   for (prop in d[part][block]) {
                       var val = d[part][block][prop];
                       if (val === "checked") {
                           var $title = $($step.find('.fw-step-part-title')[part]);
                           var title = $title.text().trim();
                           var $block = $step.find('[data-partId="' + part + '"]')
                               .find('[data-blockId="' + block + '"]');
                           var header = $block.find('.fw-block-header').text();
                           if (header) {
                               header = header.trim();
                           }
                           var value = $block.find('[data-labelId="' + prop + '"]').text();
                           if (value) {
                               value = value.trim();
                           }
                           var s = {};
    ...
    function validate($wizard) {
        var valid = true;
        var elements = $wizard.find('input[type="text"][data-required]');
        console.log(elements);
        $wizard.find('[data-required]').each(
            function (i, element) {
                console.dir(element);
                var $element = $(element);
                if (!$element.val()) {
    ...
    function setup() {
        ...
        if (doneColor) {
          ...
          $('head').append('<style>.fw-progress-step.fw-visited:before{ background:' + doneColor + ' !important; } ul.fw-progress-bar li.fw-visited:after{ background-color:'+ doneColor +'}</style>');
        }
        if (activeColor) {
          console.log('activeColor: ' + activeColor);
          $('head').append('<style>ul.fw-progress-bar li.fw-active:before{background:' + activeColor + '!important;}</style>');
        }
        if (nextColor) {
          console.log('nextColor: ' + nextColor);
          $('head').append('<style>ul.fw-progress-bar li:before{background:' + nextColor + ';}!important</style>');
        }
